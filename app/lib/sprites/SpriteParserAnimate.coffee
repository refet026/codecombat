...
module.exports = class SpriteParserAnimate
  ...
  # Steps into the parent of a node.
  walk: (node, parent, fn) ->
    node.parent = parent
        ...
        if name
          ...
          unless expression.parent?.right?.right
            if /frame_[\d]+/.test name  # Skip some useless KR function things
              return
          unless expression?.parent?.right?.right?.callee?.property?.name
            return
